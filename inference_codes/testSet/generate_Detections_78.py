...
import numpy as np
from post_processing_shape import postprocess_mask_and_markers,postprocess_mask_and_watermarkers,simple_label,remove_small_components
    ...
    for imgp in imglist:
            ...
            if da in ['DIC-C2DH-HeLa']:
                ...
                if gttype in ['GT']:
                    ...
                    center_marker_det=postprocess_mask_and_markers(center_marker_mask,seedimg,area_thresh=area_thresh_all[da][1])
                else:
                    mask = regular_norm(maskb)
            ...
            if vis:
                ...
                plt.imshow(np.concatenate([mask,center_pred,seedimg,center_marker_mask],1))
                plt.show()
                plt.imshow(center_marker_det)
                plt.show()
