...
class WeightedTorchJaccardLoss(torch.nn.modules.Module):
    # modified from XD_XD's implementation
    def __init__(self):
        super(WeightedTorchJaccardLoss, self).__init__()
    ...
    def forward(self, outputs, targets,mask):
        ...
        intersection = ((jaccard_output * jaccard_target)).sum()
        union = ((jaccard_output + jaccard_target)).sum()
        jaccard_score = ((intersection + eps) / (union - intersection + eps))
        self._stash_jaccard = jaccard_score
        loss = 1. - jaccard_score
...
class WeightedBCElogits(torch.nn.modules.Module):

    def __init__(self):
        super(WeightedBCElogits,self).__init__()
...
class rinabcewithlogits(torch.nn.modules.Module):
    def __init__(self):
        super(rinabcewithlogits, self).__init__()
...
torch_losses = {
    ...
    'rinabcewithlogits': rinabcewithlogits,
    ...
    'weightedbcelogitsrina':WeightedBCElogits,
    'weightedjaccardlossrina':WeightedTorchJaccardLoss,
