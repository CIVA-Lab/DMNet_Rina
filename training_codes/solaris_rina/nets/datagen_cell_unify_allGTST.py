...
#from ..utils.io import sio.imread, _check_channel_order
...
class TorchDataset(Dataset):
    ...
    def __init__(self, branch,df, batch_size,sizecrop,stage="train",num_classes=1):
        ...
        self.data_root_path="../Data/train/"
    ...
    def __getitem__(self, idx):
        ...
        branch=self.branch
        #print (idx,frame)
        da_name = self.df[idx][-1]
        #if da_name in ['Fluo-N3DH-CE']:
        #    print("iiiiiiiiii", self.df[idx])

        image = sio.imread(self.data_root_path+self.df[idx][0])
        #print("image size", self.data_root_path + self.df[idx][0])

        if da_name in ['Fluo-N3DH-CE','Fluo-C3DH-H157',  'Fluo-C3DH-A549', 'Fluo-C3DL-MDA231','Fluo-N3DH-CHO']:
            image=image[int(self.df[idx][-2])]

        image=contrast_strech(image)
        image = 255*equalize_adapthist(image, clip_limit=0.01)

        if branch == "center":
            #print ("mask size",self.data_root_path+self.df[idx][2])
            #print ("center")

            #if da_name in ["DIC-C2DH-HeLa"]:
               #mask = sio.imread(self.data_root_path + self.df[idx][1])
               #mask = generate_shapemarker_each(mask, thre[da_name][0], thre[da_name][1])


            #else:
            mask = sio.imread(self.data_root_path+self.df[idx][2])


            if da_name in ['Fluo-N3DH-CE', 'Fluo-C3DH-H157', 'Fluo-C3DH-A549', 'Fluo-C3DL-MDA231', 'Fluo-N3DH-CHO']:
                mask = mask[int(self.df[idx][-2])]
...
if __name__ == "__main__":
    ...
    def get_train_val_dfs_all():
        train_df = np.load('/usr/mvl2/rbync/Rinabackup/DMNet/training_codes/data_trainlist/2D3D_cell_allGTallST_train.npy',allow_pickle=True)
        val_df = np.load('/usr/mvl2/rbync/Rinabackup/DMNet/training_codes/data_trainlist/2D3D_cell_allGTallST_val.npy',allow_pickle=True)

        #print ("loading training data successfully!")
        return train_df, val_df

    train_dataset, val_dataset=get_train_val_dfs_all()

    dataset = TorchDataset("center",
                     train_dataset,
                     batch_size=1,
                     sizecrop=256,
                     num_classes=1,)

    for i in range(0,len(dataset.df)):
               print (i)
               sample=dataset[i]
