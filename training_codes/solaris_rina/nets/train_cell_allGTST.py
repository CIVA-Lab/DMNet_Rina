...
from .datagen_cell_unify_allGTST import make_data_generator
...
def get_dataset_data(alldata,dataset):
    data_use=[]
    for it in alldata:
        if it[3] in dataset:
            data_use.append(it)
...
#     print ("load name",pathload)
#     with open(pathload) as f:
#         data_t= json.load(f)
#         # if gttype == "GT+ST":
#         #     train_dataset = data_t['train_st']
#         #     val_dataset = data_t['val_st']
#         # else:
...
def get_train_val_dfs_all(config,dataset,gttype):
    train_df = np.load(config["training_data_names"],allow_pickle=True)
    val_df = np.load(config["validation_data_names"],allow_pickle=True)


    print ("loading training data successfully!")
    print ("training, validation samples length",len(train_df),len(val_df))
