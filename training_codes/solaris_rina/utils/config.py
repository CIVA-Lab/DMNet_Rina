...
def parse(path):
    ...
    if config['train'] and config['training_data_names'] is None:
        raise ValueError('"training_data_names" must be provided if training.')
    ...
    if config['pretrained'] is not None:
        config['pretrained'] = config['pretrained']
